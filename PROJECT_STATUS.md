...
(`pennywise-local-development-secret-change-me`) *unless* running in local mode with auth
...
does not by itself prove the *Supabase-hosted* database works (SSL/connection-string differences
...
`org.gradle.java.home`, so the wrapper now builds portably on any machine with a discoverable JDK
21.
...
using the new `Dockerfile`. The listener now honors `PORT` (`server.port=${PORT:8080}`, default
8080) and the API context remains `/api`.
