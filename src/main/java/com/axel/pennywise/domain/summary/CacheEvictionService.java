...
/**
 * Evicts every cached read that can go stale after a transaction/category-name write for a book.
 * Transaction writes and category renames can each touch an unbounded set of months (an edit can
 * move a transaction across a month boundary, an import can span many months), so per-month keys
 * aren't safe to target - this walks each cache and drops every entry whose key is scoped to the
 * given book instead.
 *
 * <p>Every eviction is deferred to after the enclosing transaction commits. Evicting inside the
 * transaction would open a window where a concurrent read misses the cache, re-reads the database
 * under the writer's snapshot (not-yet-committed rows aren't visible to it), and repopulates the
 * cache with the same stale values right before commit - with no further eviction scheduled, that
 * stale entry would then sit until the safety-net TTL expires.
 */
@Service
