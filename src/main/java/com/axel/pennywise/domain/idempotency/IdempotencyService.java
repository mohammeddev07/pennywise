...
public class IdempotencyService {
        ...
        .map(
            row -> {
              ...
              if (!row.getRequestHash().equals(requestHash)) {
                log.warn(
                    "Idempotency conflict: userId={}, key={}, prior_hash does not match current"
                        + " request",
                    userId,
                    key);
                return new PriorResponse(
                    409,
                    "{\"error\":{\"code\":\"IDEMPOTENCY_KEY_REUSED\",\"message\":\"Idempotency-Key"
                        + " reused with different request\"}}");
              }
              int code = row.getResponseCode() == null ? 409 : row.getResponseCode();
              log.info(
                  "Idempotency match: userId={}, key={}, returning prior response with status={}",
                  userId,
                  key,
                  code);
  ...
  public void storeResponse(
      UUID userId, String key, String requestBodyJson, int statusCode, String responseBody) {
    log.debug(
        "Storing idempotency response: userId={}, key={}, statusCode={}", userId, key, statusCode);
