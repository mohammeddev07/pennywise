...
public class BookService {
    ...
    @Transactional
    public BookEntity create(UserEntity user, String name, String currencyCode, String timezone, long openingBalanceMinor) {
        log.debug("Creating book for user: userId={}, name={}, currency={}", user.getId(), name, currencyCode);
    ...
    public BookEntity requireOwned(UUID bookId, UserEntity user) {
        ...
        return repo.findByIdAndOwner_IdAndDeletedAtIsNull(bookId, user.getId())
                .orElseThrow(() -> {
                    log.warn("Book not found or not owned by user: bookId={}, userId={}", bookId, user.getId());
