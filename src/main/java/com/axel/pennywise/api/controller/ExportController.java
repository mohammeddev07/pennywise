...
public class ExportController {
  ...
  @PostMapping("/csv")
  public ResponseEntity<ExportCreateResponse> createCsv(
    ...
    try {
      ...
      log.info(
          "Export job created: exportId={}, bookId={}, status={}",
          job.getId(),
          bookId,
          job.getStatus());

      return ResponseEntity.accepted()
          .body(new ExportCreateResponse(job.getId(), job.getStatus().name()));
    } catch (Exception e) {
  ...
  public ResponseEntity<ExportResponse> get(
      Authentication auth, @PathVariable UUID bookId, @PathVariable UUID exportId) {
  ...
  public ResponseEntity<ExportDownloadResponse> download(
      Authentication auth, @PathVariable UUID bookId, @PathVariable UUID exportId) {
    UserEntity user =
        userService.getOrCreate(
            auth, CurrentUser.subject().orElse(LOCAL), CurrentUser.email().orElse(null));
