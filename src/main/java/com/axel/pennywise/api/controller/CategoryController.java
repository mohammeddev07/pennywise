...
public class CategoryController {
  ...
  @GetMapping
  public ResponseEntity<ItemsResponse<CategoryResponse>> list(
      Authentication auth, @PathVariable UUID bookId) {
  ...
  public ResponseEntity<CategoryResponse> create(
    ...
    if (normalized == null || normalized.isBlank()) {
      throw new com.axel.pennywise.exception.ApiException(
          HttpStatus.BAD_REQUEST, VALIDATION_ERROR, "Category name is required");
    }

    boolean exists =
        categoryRepo.existsByBook_IdAndTypeAndNameIgnoreCaseAndDeletedAtIsNull(
            bookId, req.type(), normalized);
    if (exists) {
      throw new ApiException(
          HttpStatus.CONFLICT, CONFLICT, "Category name already exists for this book and type");
    }
    ...
    log.info(
        "Category created: categoryId={}, bookId={}, type={}, name={}",
        c.getId(),
        bookId,
        c.getType(),
        c.getName());
  ...
  public ResponseEntity<CategoryResponse> patch(
    ...
    CategoryEntity c =
        categoryRepo
            .findByIdAndBook_IdAndDeletedAtIsNull(categoryId, bookId)
            .orElseThrow(
                () ->
                    new com.axel.pennywise.exception.ApiException(
    ...
    if (req.isDisabled() != null) {
      log.debug(
          "Updating category disabled flag: was={}, now={}", c.isDisabled(), req.isDisabled());
    ...
    c = categoryRepo.save(c);
    log.info(
        "Category updated: categoryId={}, bookId={}, name={}, disabled={}",
        categoryId,
        bookId,
        c.getName(),
        c.isDisabled());
  ...
  public ResponseEntity<Void> delete(
    ...
    if (txRepo.existsByBook_IdAndCategory_IdAndDeletedAtIsNull(bookId, categoryId)) {
      throw new ApiException(
          HttpStatus.CONFLICT, "CATEGORY_IN_USE", "Category has active transactions");
    }
  ...
  private long parseEtagVersion(String ifMatch) {
    ...
    try {
      ...
      throw new com.axel.pennywise.exception.ApiException(
          HttpStatus.BAD_REQUEST, "INVALID_IF_MATCH", "Invalid If-Match value");
    }
  }
