...
public record TransactionListFilter(
    ...
    UUID cursorId) {
  public TransactionListFilter withDecodedCursor(
      LocalDate occurredOn, OffsetDateTime createdAt, UUID id) {
    return new TransactionListFilter(
        bookId, fromDate, toDate, type, categoryId, q, limit, cursor, occurredOn, createdAt, id);
  }
}
