...
class ExportServiceTest {
  ...
  void getDownloadUrl_throwsNotFound_whenJobMissing() {
    when(exportRepo.findByIdAndBook_IdAndDeletedAtIsNull(exportId, bookId))
        .thenReturn(Optional.empty());

    ApiException ex =
        assertThrows(ApiException.class, () -> exportService.getDownloadUrl(bookId, exportId));
  ...
  void getDownloadUrl_returnsPresignedUrl_whenCompletedAndStorageKeyPresent() {
    ...
    when(exportRepo.findByIdAndBook_IdAndDeletedAtIsNull(exportId, bookId))
        .thenReturn(Optional.of(job));
    when(presignService.presign("exports/book1/export1.csv"))
  ...
  private static String extractUrl(ExportDownloadResponse resp) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Cannot extract url from ExportDownloadResponse; add url()/getUrl() accessor.", e);
    }
  }
}
