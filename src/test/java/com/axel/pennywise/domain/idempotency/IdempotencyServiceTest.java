...
class IdempotencyServiceTest {
  ...
  void tryGetPrior_returnsEmpty_whenNoRowExists() {
    ...
    Optional<IdempotencyService.PriorResponse> resp = service.tryGetPrior(userId, key, "{\"a\":1}");
  ...
  void tryGetPrior_returnsPriorResponse_whenHashMatches_withStoredCode() {
    ...
    Optional<IdempotencyService.PriorResponse> resp = service.tryGetPrior(userId, key, requestBody);
  ...
  /**
   * Asserts IdempotencyKeyId contains the expected (userId, key) without relying on
   * equals/hashCode. Tries common getter names first; falls back to field reflection.
   */
  private static void assertIdParts(IdempotencyKeyId id, UUID expectedUserId, String expectedKey) {
    ...
    UUID actualUserId =
        (UUID)
            readPropertyOrField(id, UUID.class, "getUserId", "userId", "userId", "uid", "getUid");
    String actualKey =
        (String)
            readPropertyOrField(
                id,
  ...
  private static Object readPropertyOrField(Object target, Class<?> type, String... candidates) {
    ...
    throw new AssertionError(
        "Unable to read " + type.getSimpleName() + " from " + target.getClass().getSimpleName());
  }
}
