...
class BookControllerTest {
	...
	void testListBooks() throws Exception {
		...
		mockMvc.perform(get("/v1/books").with(auth()))
	...
	void testCreateBook() throws Exception {
		...
		when(bookService.create(eq(testUser), eq("New Book"), eq("EUR"), eq("Europe/London"), eq(5000L)))
				.thenReturn(created);

		BookCreateRequest req = new BookCreateRequest("New Book", "EUR", "Europe/London", 5000L);

		mockMvc.perform(post("/v1/books").with(auth())
	...
	void testGetBook() throws Exception {
		...
		mockMvc.perform(get("/v1/books/{bookId}", bookId).with(auth()))
				.andExpect(status().isOk())
				.andExpect(header().string("ETag", "\"0\""))
	...
	void testUpdateBook() throws Exception {
		...
		mockMvc.perform(patch("/v1/books/{bookId}", bookId).with(auth())
						.header("If-Match", "\"0\"")
						.contentType(MediaType.APPLICATION_JSON)
						.content(objectMapper.writeValueAsString(req)))
				.andExpect(status().isOk())
