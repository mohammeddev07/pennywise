...
class TransactionControllerTest {
	...
	void testListTransactions() throws Exception {
		...
		mockMvc.perform(get("/v1/books/{bookId}/transactions", bookId).with(auth()))
		...
		verifyNoMoreInteractions(userService, bookService, transactionService, categoryRepository, transactionRepository, idempotencyService);
	}
	...
	void testCreateTransaction() throws Exception {
		when(userService.getOrCreate(any(), any(), any())).thenReturn(testUser);
		when(idempotencyService.tryGetPrior(eq(testUser.getId()), anyString(), anyString())).thenReturn(Optional.empty());
		when(bookService.requireOwned(bookId, testUser)).thenReturn(testBook);
		...
		mockMvc.perform(post("/v1/books/{bookId}/transactions", bookId).with(auth())
	...
	void testCreateTransactionWithoutIdempotencyKey() throws Exception {
		...
		verifyNoInteractions(userService, bookService, categoryRepository, transactionRepository, transactionService, idempotencyService);
	}
	...
	void testGetTransaction() throws Exception {
		...
		when(transactionRepository.findByIdAndBook_IdAndDeletedAtIsNull(transactionId, bookId)).thenReturn(Optional.of(found));

		mockMvc.perform(get("/v1/books/{bookId}/transactions/{txId}", bookId, transactionId).with(auth()))
				.andExpect(status().isOk());
	...
	void testUpdateTransaction() throws Exception {
		...
		when(transactionRepository.findByIdAndBook_IdAndDeletedAtIsNull(transactionId, bookId)).thenReturn(Optional.of(existing));
		...
		mockMvc.perform(patch("/v1/books/{bookId}/transactions/{txId}", bookId, transactionId).with(auth())
						.contentType(MediaType.APPLICATION_JSON)
						.header("If-Match", "\"0\"")
						.content(objectMapper.writeValueAsString(req)))
				.andExpect(status().isOk());
	...
	void testDeleteTransaction() throws Exception {
		...
		mockMvc.perform(delete("/v1/books/{bookId}/transactions/{txId}", bookId, transactionId).with(auth())
						.header("If-Match", "\"0\""))
